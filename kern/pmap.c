...
// boot_map_region mapuje virtualnu adresu na fyzicke miesto v pamati
// velkost ktoru mapujeme pre uzivatela si musime vypocitat (vsetky env)
//  uzivatel vie iba citat (PTE_U)
// (je dost mozne, ze by fungovalo aj pre PTSIZE)
// pod tym je mapovanie pre kernel, ktory moze aj zapisovat

	size_t size = ROUNDUP(NENV * sizeof(struct Env), PGSIZE);
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	// najprv zarovname va na hranicu stranky
	const void* end = ROUNDUP(va + len, PGSIZE);

	// no fucking idea, ale inak to nefunguje
	for (; va < end; va = ROUNDDOWN(va+PGSIZE, PGSIZE)){
		pte_t *pte = pgdir_walk(env->env_pgdir, va, 0);

		// ak je adresa nad hranicou UTOP, nema k nej prostredie pristup 
		// alebo ak nie je vytvorene mapovanie
		// alebo ak nema povolenie
		// vrati sa -E_FAULT
		// nemozeme kontrolovat (*pte & PTE_P)
